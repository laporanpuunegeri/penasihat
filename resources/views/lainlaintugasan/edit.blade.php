...
<div class="container">
    ...
    <form action="{{ route('lainlaintugasan.update', $tugasan->id) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="mb-3">
            <label for="perihal" class="form-label">Perihal Tugasan</label>
            <input list="senarai-perihal" name="perihal" id="perihal" class="form-control" required value="{{ old('perihal', $tugasan->perihal) }}">
        ...
        <div class="mb-3">
            ...
            <input type="date" name="tarikh" id="tarikh" class="form-control" required
                   value="{{ old('tarikh', optional($tugasan->tarikh)->format('Y-m-d')) }}">
        </div>
