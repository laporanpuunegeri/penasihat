...
<table class="table table-bordered text-center align-middle mb-5">
    <thead class="table-secondary">
        ...
        <tr>
            <th>yang dirujuk ke AGC<br>(Ibu Pejabat)</th>
    ...
    <tbody>
        @foreach ($data as $index => $kategori)
            <tr>
                <td class="text-start">
                    <strong>{{ $loop->iteration }}.</strong> {{ $kategori['kategori'] }}
                </td>
...
{{-- Bahagian 2: Kes Mahkamah --}}
<h5 class="mt-5">2. KES MAHKAMAH <small class="text-muted">(Laporan lengkap adalah seperti di <strong>LAMPIRAN II</strong>)</small></h5>

<div class="table-responsive">
    <table class="table table-bordered align-middle text-center">
        <thead class="table-secondary">
            <tr>
                <th rowspan="2" style="width: 20%">Pembahagian kes mengikut isu</th>
                <th rowspan="2">Bilangan Masih Aktif</th>
                <th colspan="5">Mahkamah</th>
        ...
        <tbody>
            @php
                $jumlah = ['aktif' => 0, 'maj' => 0, 'sesi' => 0, 'tinggi' => 0, 'rayuan' => 0, 'persk' => 0];
            @endphp
            ...
            @forelse ($lampiran_kesmahkamah as $row)
                ...
                @php
                    $jumlah['aktif'] += $row->bil_aktif;
                    $jumlah['maj'] += $row->majistret;
                    $jumlah['sesi'] += $row->sesi;
                    $jumlah['tinggi'] += $row->tinggi;
                    $jumlah['rayuan'] += $row->rayuan;
                    $jumlah['persk'] += $row->persk;
                @endphp
            @empty
                <tr>
                    <td colspan="8" class="text-muted">Tiada data Lampiran Kes Mahkamah untuk bulan ini.</td>
                </tr>
            @endforelse
...
document.addEventListener('DOMContentLoaded', function () {
    const inputs = document.querySelectorAll('.jumlah-kes-input');
    const totalInputs = document.querySelectorAll('.jumlah-keseluruhan');

    function calculateTotals() {
        for (let col = 0; col < 7; col++) {
            let total = 0;
            document.querySelectorAll(`.jumlah-kes-input:nth-child(${col + 2})`).forEach(input => {
                total += parseInt(input.value) || 0;
            });
            totalInputs[col].value = total;
        }
    }
    ...
    calculateTotals(); // auto trigger awal
});
</script>
