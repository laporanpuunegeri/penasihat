...
<div class="container-fluid px-4 py-4">
    ...
    <div class="page-header d-flex flex-wrap justify-content-between align-items-center gap-3">
        <div>
            <h3 class="mb-1 fw-bold text-dark">Laporan Semakan Undang-Undang</h3>
            <p class="text-muted small mb-0">Senarai penyemakan RUU, Perundangan Subsidiari & Pemberitahuan Awam.</p>
        </div>
        
        <div class="d-flex gap-2">
            <a href="{{ route('laporansemakanundang.create') }}" class="btn btn-primary shadow-sm px-4">
                <i class="fas fa-plus-circle me-2"></i> Daftar Semakan
            </a>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body py-3 px-4">
            <form method="GET" action="{{ route('laporansemakanundang.index') }}" class="row align-items-center g-3">
                <div class="col-auto">
                ...
                @if(request('bulan'))
                <div class="col-auto">
                    <a href="{{ route('laporansemakanundang.index') }}" class="btn btn-outline-secondary btn-sm rounded-pill px-3">
            ...
            <table class="table table-hover align-middle text-center mb-0">
                <thead>
                    <tr>
                        ...
                        <th width="35%" class="text-start">Tajuk RUU / Perundangan</th>
                        <th width="25%" class="text-start">Tindakan</th>
                ...
                <tbody>
                    ...
                    @forelse ($data as $index => $laporan)
                        <tr>
                            ...
                            {{-- Status --}}
                            <td>
                                @if(Str::contains(strtolower($laporan->status), ['lulus', 'selesai', 'kuat kuasa', 'warta']))
                                    <span class="badge bg-success status-badge">
                                        <i class="fas fa-check-circle me-1"></i> {{ $laporan->status }}
                                    </span>
                                @elseif(Str::contains(strtolower($laporan->status), ['semakan', 'sedia']))
                                    <span class="badge bg-info text-dark status-badge">
                                        <i class="fas fa-search me-1"></i> {{ $laporan->status }}
                                    </span>
                                @else
                            ...
                            <td>
                                <div class="d-flex justify-content-center gap-2">
                                    {{-- Edit --}}
                                    <a href="{{ route('laporansemakanundang.edit', $laporan->id) }}" 
                                    ...
                                    @if ($currentUser->id === $laporan->user_id || $currentUser->role === 'admin')
                                        <form action="{{ route('laporansemakanundang.destroy', $laporan->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Anda pasti untuk padam rekod ini?');">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-5 text-muted bg-light">
                                ...
                                <p class="mb-0 fw-bold">Tiada rekod semakan ditemui.</p>
                                <small>Sila klik butang "Daftar Semakan" untuk mula.</small>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
