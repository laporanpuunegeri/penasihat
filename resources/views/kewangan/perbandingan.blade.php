                ...
                <table class="table table-bordered table-hover mb-0 text-dark" width="100%" cellspacing="0">
                    ...
                    <thead>
                        <tr class="text-center">
                            ...
                            <th rowspan="2" style="vertical-align: middle; background-color: #4e73df; color: white; border: 1px solid #fff; width: 35%;">BUTIRAN PERBELANJAAN</th>
                            <th colspan="2" class="text-uppercase font-weight-bold" style="background-color: #4e73df; color: white; border: 1px solid #fff;">JUMLAH BELANJA (RM)</th>
                            <th colspan="2" class="text-uppercase font-weight-bold" style="background-color: #4e73df; color: white; border: 1px solid #fff;">PERBEZAAN (+/-)</th>
                        </tr>
                        <tr class="text-center">
                            <th style="background-color: #2e59d9; color: white; border: 1px solid #fff; width: 15%;">TAHUN {{ $tahun_lepas }}</th>
                            <th style="background-color: #2e59d9; color: white; border: 1px solid #fff; width: 15%;">TAHUN {{ $tahun_semasa }}</th>
                            
                            <th style="background-color: #f6c23e; color: #000; border: 1px solid #fff; width: 15%;">RM</th>
                            <th style="background-color: #f6c23e; color: #000; border: 1px solid #fff; width: 10%;">%</th>
                        </tr>
                    </thead>
                    ...
                    <tbody>
                        ...
                        @foreach($laporan as $kod_utama => $data)
                            <tr style="background-color: #eaecf4; border-top: 2px solid #d1d3e2;">
                                ...
                                <td colspan="5" class="font-weight-bold text-dark text-uppercase">{{ $data['tajuk'] }}</td>
                            </tr>
                            ...
                            @if(count($data['items']) > 0)
                                @foreach($data['items'] as $item)
                                    @php
                                        ...
                                        // Warna Indikator (Hijau = Naik, Merah = Turun)
                            ...
                            @else
                                <tr><td colspan="6" class="text-center text-muted font-italic py-2">Tiada rekod.</td></tr>
                            @endif
                        @endforeach
                        ...
                        <tr style="background-color: #2e59d9; color: white; border-top: 3px solid #224abe;">
                            <td colspan="2" class="text-right font-weight-bold py-3 text-uppercase">JUMLAH BESAR:</td>
                            <td class="text-right font-weight-bold py-3">{{ number_format($grand_lepas, 2) }}</td>
                            <td class="text-right font-weight-bold py-3">{{ number_format($grand_semasa, 2) }}</td>
                            
                            <td class="text-right font-weight-bold py-3" style="background-color: #f6c23e; color: black;">
                                {{ $grand_diff > 0 ? '+' : '' }}{{ number_format($grand_diff, 2) }}
                            </td>
                            <td class="text-center font-weight-bold py-3" style="background-color: #f6c23e; color: black;">
