...
<div class="container-fluid px-4 py-4">
    ...
    <div class="page-header d-flex justify-content-between align-items-center">
        <div>
            <h3 class="mb-1 fw-bold text-dark">Kemaskini Pindaan</h3>
            <p class="text-muted small mb-0">Kemaskini maklumat Pindaan RUU atau Perundangan Subsidiari.</p>
        </div>
        <div class="d-flex gap-2">
            {{-- Butang Padam --}}
            @if(auth()->user()->id === $laporan->user_id || auth()->user()->role === 'admin')
                <form action="{{ route('laporanpindaanundang.destroy', $laporan->id) }}" method="POST" onsubmit="return confirm('Adakah anda pasti mahu memadam rekod ini?');">
                    @csrf @method('DELETE')
    ...
    <form method="POST" action="{{ route('laporanpindaanundang.update', $laporan->id) }}">
        ...
        <div class="row">
            ...
            {{-- KOLUM KIRI: MAKLUMAT PINDAAN --}}
            <div class="col-lg-8">
                <div class="form-card p-4 mb-4">
                    <h5 class="form-section-title"><i class="fas fa-file-pen me-2 text-warning"></i> Butiran Pindaan</h5>
