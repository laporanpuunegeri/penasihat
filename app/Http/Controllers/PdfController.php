...
class PdfController extends Controller
{
    public function laporan(Request $request)
    {
        ...
        $lampiranKes = $lampiran->map(fn($item) => [
            'bil_aktif' => $item->bil_aktif ?? 0,
            'majistret' => $item->majistret ?? 0,
            'sesi'      => $item->sesi ?? 0,
            'tinggi'    => $item->tinggi ?? 0,
            'rayuan'    => $item->rayuan ?? 0,
            'persk'     => $item->persk ?? 0,
            'status'    => $item->status ?? '-',
        ])->toArray();
        ...
        // Jumlah Keseluruhan
        $jumlahKeseluruhan = [
            'bil_aktif' => $lampiran->sum('bil_aktif'),
            'majistret' => $lampiran->sum('majistret'),
            'sesi'      => $lampiran->sum('sesi'),
            'tinggi'    => $lampiran->sum('tinggi'),
            'rayuan'    => $lampiran->sum('rayuan'),
            'persk'     => $lampiran->sum('persk'),
        ];
